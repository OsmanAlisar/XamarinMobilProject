...
namespace OsmanAlisarIdesse.Provider
{
    internal class ServiceManager
    {
        ...
        public async Task<SMResult> Login(string userName,string password)
        {
          ...
          await  GetClient();
            //var auData = string.Format("{0}:{1}", usr.username, usr.password);
            //var authHeaderVal = Convert.ToBase64String(Convert.FromBase64String(auData));   
        ...
        public async Task<SMResult> GetListMobile(int limit, int start, bool includeVisitStats)
        {
            ...
            //var responsen = client.GetAsync(basaeUrl + @"Card/GetListMobile?Limit="+limit.ToString()+"&Start="+start.ToString()).Result;
            ...
            result.ResultData = JsonConvert.DeserializeObject<ObservableCollection<MobileDataModel>>(rp.data.ToString());
